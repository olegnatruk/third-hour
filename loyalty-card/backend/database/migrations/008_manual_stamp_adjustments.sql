-- Apply after 007_rewards_and_redemptions.sql.
...
begin
  ...
  values (
    v_card.id, p_stamp_change, 'manual_adjustment', trim(p_reason), p_actor_id, p_idempotency_key
  );
...
revoke all on function public.adjust_active_card_stamp(uuid, uuid, integer, text, uuid) from public;
grant execute on function public.adjust_active_card_stamp(uuid, uuid, integer, text, uuid) to service_role;
